 ...
 *
 * Runs automatically on script load. If a page re-renders its sidebar
 * dynamically (e.g. walkthrough.html adds/renames room instances),
 * call window.initTocSidebars() again to rebind observers to new DOM.
 *
...
(function () {
  var bindings = []; // [{ observer, firstLink, clickHandler }]

  function teardown() {
    bindings.forEach(function (b) {
      if (b.observer) b.observer.disconnect();
      if (b.firstLink && b.clickHandler) {
        b.firstLink.removeEventListener('click', b.clickHandler);
      }
    });
    bindings = [];
  }
  ...
  function init() {
    ...
    sidebars.forEach(function (sidebar) {
      ...
      bindings.push({ observer: observer, firstLink: firstLink, clickHandler: clickHandler });
    });
  }

  window.initTocSidebars = init;
  init();
})();
